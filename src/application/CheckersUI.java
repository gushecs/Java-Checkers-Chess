...
public class CheckersUI {
	...
	// https://stackoverflow.com/questions/5762491/how-to-print-color-in-console-using-system-out-println
		...
		public static void printBoard(CheckersPiece[][] pieces, boolean[][] possibleMoves,CheckersMatch checkersMatch) {
			for (int i = 0; i < pieces.length; i++) {
				...
				for (int j = 0; j < pieces.length; j++) {
					...
					System.out.print(bg);
					printPiece(pieces[i][j], possibleMoves[i][j],checkersMatch.getCurrentPlayer(),whiteBlackIndex);
				}
				System.out.print(ANSI_BLACK_BACKGROUND);
				System.out.println();
			}
		...
		private static void printPiece(CheckersPiece piece, boolean background, Color currentPlayer, int whiteBlackIndex) {
			String bgc = " ";
